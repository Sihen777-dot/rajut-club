...
// Hanya COD (cash) dan Transfer — nilai lama tetap diterima agar data lama tidak error
const METODE_PEMBAYARAN_VALID = ["cash", "transfer_bank", "qris", "bayar_ditempat"];
...
const updateMyPembelian = async (req, res) => {
  try {
    ...
    const { metode_pembayaran, pembayaran, status, foto_bukti, jumlah } = req.body;
    const payload = {};

    // Pembeli boleh bayar (cash/qris) → set Dibayar
    ...
    if (pembayaran) {
      if (!["Belum_dibayar", "Dibayar"].includes(pembayaran)) {
        return res.status(400).json({ message: "Status pembayaran tidak valid" });
      }
      payload.pembayaran = pembayaran;
    }
    // Pembeli boleh: konfirmasi diterima, selesai, atau batalkan pesanan
    if (status) {
      if (!["Menunggu", "Diterima", "Selesai", "Dibatalkan"].includes(status)) {
        return res.status(400).json({ message: "Status tidak diizinkan untuk pembeli" });
      }
      // Batalkan hanya jika belum selesai / belum diterima
      if (status === "Dibatalkan") {
        const cur = String(existing.status || "").toLowerCase();
        if (cur === "selesai" || cur === "diterima") {
          return res.status(400).json({
            message: "Pesanan yang sudah diterima/selesai tidak bisa dibatalkan",
          });
        }
      }
