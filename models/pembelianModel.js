  ...
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT
        ...
        COUNT(*) FILTER (WHERE status = 'Diterima') AS total_diterima,
  ...
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT
        ...
        (SELECT COUNT(*) FILTER (WHERE status IN ('Menunggu', 'Diterima')) FROM pembelian) AS pesanan_aktif,
        (SELECT COUNT(*) FILTER (WHERE status_bayar = 'Belum_dibayar') FROM pembelian) AS belum_dibayar,
        (SELECT COALESCE(SUM(
