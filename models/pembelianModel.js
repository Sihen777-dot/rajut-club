  ...
  return new Promise((resolve, reject) => {
    const sql = `
      SELECT
        ...
        (SELECT COALESCE(SUM(status IN ('Menunggu', 'Diterima')), 0) FROM pembelian) AS pesanan_aktif,
        (SELECT COALESCE(SUM(status_bayar = 'Belum_dibayar'), 0) FROM pembelian) AS belum_dibayar,
        (SELECT COALESCE(SUM(
